 ...
 * - Bulletproof iOS/iPadOS Safari audio unlocker
 */
...
export class SoundEngine {
  ...
  public attachUserGestureUnlock(): void {
    const unlock = () => {
      // 1. Force iOS/iPadOS Safari CoreAudio session to Media Playback
      try {
      ...
      // 2. Initialize and wake AudioContext
      this.init();
  ...
  public init(): void {
    ...
    // Direct iOS hardware speaker activation burst
    try {
  ...
  public setMuted(muted: boolean): void {
    ...
    if (this.masterGain && this.ctx) {
      const now = Math.max(this.ctx.currentTime, 0.01) + 0.01;
      this.masterGain.gain.setValueAtTime(this.isMuted ? 0 : 1.0, now);
    }
    if (this.isMuted) {
   ...
   * Guaranteed 100% reliable zero-fail audio synthesis on all devices & iPad Safari
   */
  public playCrossingBell(): void {
    ...
    gain1.gain.linearRampToValueAtTime(0.65, now + 0.006);
  ...
  public startTrainSound(): void {
    ...
    const playJointHit = () => {
      ...
      setTimeout(() => {
        if (this.ctx) playBogie(Math.max(this.ctx.currentTime, 0.01) + 0.01, 0.18, 100);
      }, 65);
    };
  ...
  public playStationDepartureMelody(): void {
    ...
    const now = Math.max(this.ctx.currentTime, 0.01) + 0.02;
    notes.forEach(n => {
      const osc = this.ctx!.createOscillator();
      const gain = this.ctx!.createGain();
